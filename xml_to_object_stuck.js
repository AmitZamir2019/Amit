...
function GetFullPathAttributesArray(xmlDoc) {
    ...
    {
        console.log(arr_tags[i]);
         // Check if this tag is the first:
        let has_parent = IsNotNull(arr_tags[i].parentElement);

        let path;
        let not_parent = arr_tags[i].children.length == 0;
        
        path = arr_tags[i].tagName;

        while(has_parent){
            path = arr_tags[i].parentElement.tagName + ">" + path; 
            tag = arr_tags[i].parentElement;
            has_parent = IsNotNull(arr_tags[i].parentElement);
        ...
        paths_with_attributes = paths_with_attributes.filter(x => IsNotUndefined(x));
